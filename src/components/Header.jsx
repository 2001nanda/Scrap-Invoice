    ...
    <div className="mb-6 pb-4 border-b-2 border-gray-800">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        <div className="flex flex-col gap-1">
          <label className="text-xs text-gray-500 uppercase tracking-wide no-print">Mobile Number</label>
          <input
            className="border border-gray-300 rounded-lg px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 no-print"
            value={mobileNumber}
            onChange={e => setMobileNumber(e.target.value)}
            placeholder="Enter mobile number"
          />
          <span className="hidden print:inline text-sm text-gray-700">{mobileNumber}</span>
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-xs text-gray-500 uppercase tracking-wide no-print">Address</label>
          <input
            className="border border-gray-300 rounded-lg px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 no-print"
            value={address}
          ...
          />
          <span className="hidden print:inline text-sm text-gray-700">{address}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-2">
        <button
          ...
          className="text-xs bg-purple-50 border border-purple-300 text-purple-700 px-3 py-1.5 rounded-lg font-medium hover:bg-purple-100 transition no-print w-fit"
        >
          {showQrInput ? '✕ Hide QR' : '+ Add QR Code'}
        </button>

        {showQrInput && (
          <div className="flex flex-col gap-2 p-3 bg-purple-50 border border-purple-200 rounded-lg no-print">
        ...
        {qrCode && (
          <div className="flex flex-col items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
            <p className="text-xs text-gray-600 font-medium">QR Code Preview</p>
            <QRCodeSVG value={qrCode} size={120} level="H" includeMargin={true} />
          </div>
        )}
      </div>
